...
if __name__ == '__main__':
    def output_fcn(self):
        now = 'ind_mac_nonlinear_without_startup'
    ...
    machine_2 = im_3kW.InductionMachine(nonlinear=True, pwm=True, grid='im_3kW_4k',
                                        t_start=0, t_stop=2 ** -3, nt=2 ** 12 + 1)
    ...
    machine_4 = im_3kW.InductionMachine(nonlinear=True, pwm=True, grid='im_3kW_4k',
                                        t_start=0, t_stop=2 ** -3, nt=2 ** 10 + 1)
    machine_5 = im_3kW.InductionMachine(nonlinear=True, pwm=False, grid='im_3kW_4k',
                                        t_start=0, t_stop=2 ** -3, nt=2 ** 9 + 1)
    machine_6 = im_3kW.InductionMachine(nonlinear=True, pwm=False, grid='im_3kW_4k',
                                        t_start=0, t_stop=2 ** -3, nt=2 ** 8 + 1)
    machine_7 = im_3kW.InductionMachine(nonlinear=True, pwm=False, grid='im_3kW_4k',
                                        t_start=0, t_stop=2 ** -3, nt=2 ** 7 + 1)
    machine_8 = im_3kW.InductionMachine(nonlinear=True, pwm=False, grid='im_3kW_4k',
                                        t_start=0, t_stop=2 ** -3, nt=2 ** 6 + 1)

    problem = [machine_0, machine_1, machine_2, machine_3, machine_4, machine_5, machine_6, machine_7, machine_8]
    ...
    # problem = [machine_6]
    # transfer = []
    mgrit = mgrit.Mgrit(problem=problem,transfer=transfer, it=10, nested_iteration=True, tol=1,
