...
def main():
    def output_fcn(self):
        ...
        np.save(path + '/advection-' + str(self.comm_time_rank),
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        ...
        x = advection_lvl_0.x
        fig, ax = plt.subplots()
        ax.plot(x, sol_flat[0].get_values(), 'b:', label='initial condition u(x, 0)')
        ax.plot(x, sol_flat[-1].get_values(), 'r-', label='solution at final time u(x, 2)')
